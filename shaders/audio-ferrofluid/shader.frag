// Ferrofluid speaker — glossy black magnetic fluid spikes & droplets.
// Live uniforms (when panel role is audio-beat): u_beat, u_envelope, u_beat_phase, u_bass.
...
// Height added beyond the core pool for a given angle (polar ferrofluid spikes).
float spikeHeight(float ang, float t, float beat, float bass, float env) {
    ...
    // Primary lobes — classic multi-spike ferrofluid crown
    for (int i = 0; i < 16; i++) {
        float fi = float(i);
        if (fi >= count) break;
        ...
        // Slow individual drift so spikes "swim"
        ...
        // Angular width: sharper = narrower needles
        ...
        // Audio: bass thickens mass, beat launches spikes, envelope sustains
        amp *= 0.4 + beat * 1.05 + bass * 0.55 + env * 0.35;
        // Alternate spikes fire harder (asymmetric magnetic response)
        amp *= 0.65 + 0.35 * sin(u_beat_phase * 2.1 + fi * 1.7);
        h += lobe * amp;
    }

    // Secondary fine needles between primaries
    float fine = 0.0;
    for (int j = 0; j < 8; j++) {
        float fj = float(j);
    ...
    // Soft angular grain so the silhouette never freezes
...
float ferroSDF(vec2 p, float t, float beat, float bass, float env) {
    ...
    // Bulbous spike tips (metaball caps) — matches the soft blobs in the photo
    ...
    for (int i = 0; i < 12; i++) {
        ...
        // Tips only matter when spikes are up
...
void main() {
    ...
    // Idle residual motion when silent so the fluid still looks alive
    ...
    // Edge fresnel along the silhouette
    ...
    fluid *= 0.28 + 0.72 * ndl;
    fluid += u_highlight * spec * (0.5 + beat * 0.4);
    fluid += mix(u_highlight, vec3(0.5, 0.65, 0.95), 0.4) * fres * 0.5;
    // Depth toward center
    fluid *= 0.72 + 0.28 * smoothstep(0.0, 0.22, length(p));
    fluid += u_highlight * beat * 0.07 * body;

    // --- chamber glass / metal (product photo silver) ---
    float rr = length(p);
    vec3 chamber = u_chamber;
    chamber *= 0.9 + 0.1 * n2(p * 2.8 + t * 0.04);
    chamber = mix(chamber * 0.72, chamber * 1.06, smoothstep(0.5, 0.0, rr));
    chamber += u_fluid_color * soft * 0.035 * (0.25 + env);

    vec3 col = mix(chamber, fluid, body);
    float contact = smoothstep(0.07, 0.0, d) * (1.0 - body);
    col *= 1.0 - contact * 0.16;

    // Circular viewport like the ferrofluid speaker window
    // u_window: 0 = full frame, 1 = tight circle
    ...
    vec3 outside = u_bg_color * (0.88 + 0.12 * (1.0 - uv.y));
