...
class LoadingManager {
    constructor() {
        ...
        // Embed / host pages have no loading DOM — no-op safely
        this.active = !!(this.loadingText || this.loadingScreen);
        
        // Set initial display values
        if (this.loadingText) this.loadingText.textContent = 'INITIALIZING ARTEF4KT...';
        if (this.loadingPercentage) this.loadingPercentage.textContent = '5%';
        if (this.loadingProgressBar) {
            this.loadingProgressBar.style.width = '5%';
        }
        
        if (this.active) this.animateProgress();
    }
        ...
        if (!customText) {
            for (let i = this.currentStepIndex; i < this.loadingSteps.length; i++) {
                if (this.targetProgress >= this.loadingSteps[i].progress) {
                    if (this.loadingText) this.loadingText.textContent = this.loadingSteps[i].text;
...
// Initialize loading manager (safe when loading-screen DOM is absent — embed mode)
window.loadingManager = new LoadingManager();

class FerrofluidVisualizer {
    /**
     * @param {object} [options]
     * @param {boolean} [options.embed] - music_view / headless host (no UI chrome, no own audio)
     * @param {HTMLCanvasElement} [options.canvas] - canvas to render into (embed)
     * @param {number} [options.width]
     * @param {number} [options.height]
     * @param {boolean} [options.externalAnalysis] - analysis driven by host (setExternalAnalysis)
     */
    constructor(options = {}) {
        const opts = options && typeof options === 'object' ? options : {};
        this.embedMode = !!opts.embed;
        this.externalAnalysis = !!(opts.externalAnalysis || opts.embed);
        this.embedWidth = Math.max(1, Math.round(Number(opts.width) || 400));
        this.embedHeight = Math.max(1, Math.round(Number(opts.height) || 400));

        // Update loading progress (standalone only)
        if (window.loadingManager && typeof window.loadingManager.updateProgress === 'function') {
            window.loadingManager.updateProgress(10);
        }
        
        this.canvas = opts.canvas || document.getElementById('visualizer');
        
        if (!this.canvas) {
            console.error('❌ Canvas not found! Check id="visualizer" or pass options.canvas');
            return;
        }
        
        console.log(this.embedMode ? '✅ ARTEF4KT embed canvas' : '✅ Canvas found:', this.canvas);
        ...
        // Light enable flags (music_view Controls + embed)
        this.lightsEnabled = true;       // master: whole light group
        this.lightKeyEnabled = true;     // main / fill / rim / ambient / spot
        this.lightBassEnabled = true;
        this.lightMidEnabled = true;
        this.lightHighEnabled = true;
          // Three.js setup
        this.scene = null;
        this.camera = null;
        this.renderer = null;
        ...
        if (this.embedMode) {
            // Host owns audio + chrome; only boot the 3D scene
            this.init();
            // Camera controls object is required by the animate loop; skip document listeners
            this.initEmbedCameraControls();
            this.animate();
            return;
        }
    ...
    /**
     * Inject analysis from music_view audio-frame (embed mode).
     * @param {{ bass?: number, mid?: number, high?: number, beat?: number, envelope?: number, sensitivity?: number }} data
     */
    setExternalAnalysis(data) {
        if (!data || typeof data !== 'object') return;
        const s = Number.isFinite(Number(data.sensitivity)) ? Number(data.sensitivity) : this.sensitivity;
        const clamp01 = (v) => {
            const n = Number(v);
            if (!Number.isFinite(n)) return 0;
            return n < 0 ? 0 : n > 1 ? 1 : n;
        };
        // Match standalone weighting roughly (see analyzeAudio)
        this.bassIntensity = clamp01(data.bass) * s * 0.8;
        this.midIntensity = clamp01(data.mid) * s * 1.2;
        this.highIntensity = clamp01(data.high) * s * 1.1;
        // Optional: fill dataArray with synthetic spectrum for systems that sample bins
        if (this.dataArray && this.dataArray.length) {
            const bass = clamp01(data.bass) * 255;
            const mid = clamp01(data.mid) * 255;
            const high = clamp01(data.high) * 255;
            const beat = clamp01(data.beat) * 255;
            const n = this.dataArray.length;
            for (let i = 0; i < n; i++) {
                const t = i / n;
                let v = 0;
                if (t < 0.12) v = bass;
                else if (t < 0.45) v = mid;
                else v = high;
                // Pulse highs slightly on beat
                v = Math.min(255, v + beat * (t > 0.5 ? 0.35 : 0.15));
                this.dataArray[i] = v | 0;
            }
        }
        // Drive embed info overlay bars (if DOM was injected by host)
        if (typeof this.updateFrequencyIndicators === 'function') {
            try { this.updateFrequencyIndicators(); } catch (_) { /* ignore */ }
        }
    }

    /** Resize renderer to host container (embed). */
    setEmbedSize(width, height) {
        this.embedWidth = Math.max(1, Math.round(Number(width) || this.embedWidth));
        this.embedHeight = Math.max(1, Math.round(Number(height) || this.embedHeight));
        this.resize();
    }
    ...
    getRandomSpawnEmoji() {
        ...
        const progress = (n) => {
            if (lm && typeof lm.updateProgress === 'function') lm.updateProgress(n);
        };
        progress(25);
        ...
        try {
            ...
            // Sync UI controls with default settings (no-ops when controls missing)
            if (typeof this.filmicToneSystem.syncUIControls === 'function') {
                this.filmicToneSystem.syncUIControls();
            }
            
            // Set up filmic controls event listeners only in standalone UI
            if (!this.embedMode && typeof this.setupFilmicControls === 'function') {
        ...
        // Performance monitor (optional; UI indicators may be absent in embed)
        try {
            this.performanceMonitor = new PerformanceMonitor(this);
            console.log('PerfMonitor init OK');
        } catch (e) {
            console.warn('PerfMonitor skipped', e);
    ...
    setupThreeJS() {
        ...
        // Far fog plane high enough for deep zoom-out (was 100; max camera can exceed that)
        this.scene.fog = new THREE.Fog(this.backgroundColor, 40, 220);
        
        // Camera
        const initW = this.embedMode ? this.embedWidth : window.innerWidth;
        const initH = this.embedMode ? this.embedHeight : window.innerHeight;
        this.camera = new THREE.PerspectiveCamera(
            75,
            Math.max(0.01, initW / Math.max(1, initH)),
            0.1,
            1000
        );
        // Head-on start: on +X axis looking at origin (matches cameraControls azimuth=0, elevation=0)
        const initialDistance = this.getBaseDistance();
        this.camera.position.set(initialDistance, 0, 0);
        this.camera.lookAt(0, 0, 0);
          // Renderer — preserveDrawingBuffer so music_view can capture into postprocess
        this.renderer = new THREE.WebGLRenderer({
            canvas: this.canvas,
            antialias: true,
            alpha: false,
            preserveDrawingBuffer: !!this.embedMode
        });
        this.renderer.setSize(initW, initH, !this.embedMode);
        this.renderer.setPixelRatio(Math.min(window.devicePixelRatio || 1, this.embedMode ? 1.5 : 2));
      ...
      createLighting() {
        ...
        if (this.scene.fog) {
            if (this.envVisibility === 0) {
                ...
                // Far plane allows deep zoom-out without the scene fading to black
    ...
    updatePerformanceDisplay() {
        if (!this.performanceMonitor || this.embedMode) return;
        ...
        fpsEl.textContent = `${Math.round(stats.avgFPS)}`;
        ...
        const qEl = document.getElementById('performance-quality');
        if (qEl) {
            qEl.textContent = `${qualityDisplayNames[stats.currentQuality] || stats.currentQuality}`;
        }
        ...
        const oEl = document.getElementById('performance-objects');
        if (oEl) {
            oEl.textContent = `${totalObjects}`;
        }
    ...
    analyzeAudio() {
        // Host-driven analysis (music_view): intensities already set via setExternalAnalysis
        if (this.externalAnalysis) {
            if (!this.isPlaying) {
            ...
            // Still run BPM from current intensities when playing
            if (this.isPlaying) {
                try { this.detectBPM(); } catch (_) { /* ignore */ }
            }
            return;
        }
        ...
        const freqEl = document.getElementById('track-freq-display');
        if (freqEl) {
            freqEl.textContent = `${Math.round(dominantFrequency)} Hz`;
        }
          // BPM Detection
        this.detectBPM();        
...
for (const bd of blobs) {
    ...
    updateLighting() {
        ...
        // Master kill switch
        if (this.lightGroup) {
            this.lightGroup.visible = this.lightsEnabled !== false;
        }
        if (this.lightsEnabled === false) {
            // Zero intensities so nothing leaks if something else samples them
            this.colorLights.forEach((l) => { if (l) l.intensity = 0; });
            return;
        }

        const bandOn = [
            this.lightBassEnabled !== false,
            this.lightMidEnabled !== false,
            this.lightHighEnabled !== false,
        ];
        ...
        const bands = [currentBass, currentMid, currentHigh];
        const colors = [this.lightBassColor, this.lightMidColor, this.lightHighColor];
        ...
        for (let i = 0; i < 3; i++) {
            ...
            light.visible = bandOn[i];
            light.intensity = bandOn[i] ? bands[i] * 2 : 0;
        }
        
        // More dynamic light movement
        this.colorLights.forEach((light, index) => {
            if (!light || !bandOn[index]) return;
        ...
        const keyOn = this.lightKeyEnabled !== false;
        // Indices: 0 main, 1 fill, 2 rim, 3 ambient, 4-6 color, 7 spot
        if (this.lightGroup) {
            const keyIdx = [0, 1, 2, 3, 7];
            for (const idx of keyIdx) {
                const L = this.lightGroup.children[idx];
                if (!L) continue;
                L.visible = keyOn;
            }
            if (this.lightGroup.children[0] && keyOn) { // Main light
                this.lightGroup.children[0].intensity = 2.0 + totalActivity * 0.5;
            } else if (this.lightGroup.children[0]) {
                this.lightGroup.children[0].intensity = 0;
            }
            // Dim ambient when key lights off so scene is not fully black-unreadable
            // (user can still kill everything via lightsEnabled)
            if (this.lightGroup.children[3] && this.lightGroup.children[3].type === 'AmbientLight') {
                if (!keyOn) {
                    this.lightGroup.children[3].intensity = 0;
                }
                // when key on, intensity is managed by updateLightingFromBackground / defaults
            }
        }
    }

    /** Apply enable flags immediately (without waiting for next audio frame). */
    applyLightEnableState() {
        if (this.lightGroup) {
            this.lightGroup.visible = this.lightsEnabled !== false;
        }
        if (this.colorLights) {
            ...
            this.colorLights.forEach((light, i) => {
                if (!light) return;
                light.visible = this.lightsEnabled !== false && bandOn[i];
                if (!light.visible) light.intensity = 0;
            });
        }
        if (this.lightGroup) {
            const keyOn = this.lightsEnabled !== false && this.lightKeyEnabled !== false;
            // Default base intensities for key stack (main, fill, rim, ambient, spot)
            const defaults = { 0: 2.0, 1: 0.8, 2: 1.2, 3: 0.4, 7: 1.5 };
            [0, 1, 2, 3, 7].forEach((idx) => {
                const L = this.lightGroup.children[idx];
                if (!L) return;
                L.visible = keyOn;
                if (!L.userData) L.userData = {};
                if (L.userData._baseIntensity == null && L.intensity > 0) {
                    L.userData._baseIntensity = L.intensity;
                }
                if (!keyOn) {
                    L.intensity = 0;
                } else {
                    L.intensity = L.userData._baseIntensity != null
                        ? L.userData._baseIntensity
                        : (defaults[idx] != null ? defaults[idx] : L.intensity);
                }
            });
        }
        try {
            if (this.lightsEnabled !== false && this.lightKeyEnabled !== false
                && typeof this.updateLightingFromBackground === 'function') {
                this.updateLightingFromBackground();
            }
            this.updateLighting();
        } catch (_) { /* ignore */ }
    ...
    updateUIOpacity() {        // Apply opacity to all onscreen UI elements
        ...
        uiElements.forEach(element => {
            if (element) {
                // Embed host may force-hide individual overlays (Controls toggles)
                if (element.dataset && element.dataset.artef4ktOverlay === '0') {
    ...
    }    /** Embed: cameraControls + optional canvas-local orbit (does not use document listeners). */
    initEmbedCameraControls() {
        this.mouse = { x: 0, y: 0, prevX: 0, prevY: 0 };
        this.mousePressed = false;
        this.raycaster = new THREE.Raycaster();
        this.mouseVector = new THREE.Vector2();
        /** When true, left-drag / wheel on the canvas orbits (embed). Default on for manual control. */
        this.cameraOrbitEnabled = true;
        // Head-on static pose by default (no auto orbit / music camera)
        const headOnDist = this.getBaseDistance();
        this.cameraControls = {
            distance: headOnDist,
            azimuth: 0,
            elevation: 0, // true head-on (not elevated side orbit)
            targetX: 0,
            targetY: 0,
            targetZ: 0,
            ...
            maxElevation: 60,
            damping: 0.1,
            minX: -this.gridSize + 3,
            maxX: this.gridSize - 3,
            minY: -8,
            maxY: this.gridSize * 1.5,
            minZ: -this.gridSize + 3,
            maxZ: this.gridSize - 3,
            autoMovement: {
                enabled: false, // default: static — no music-driven orbit
                baseAzimuth: 0,
                baseElevation: 0,
                baseDistance: headOnDist,
                rotationSpeed: 0.8,
                zoomIntensity: 2,
                elevationIntensity: 5,
                lastBeatTime: 0,
                beatThreshold: 0.15,
                rotationPhase: 0,
        ...
        if (this.embedMode && this.canvas) {
            this.bindEmbedCanvasOrbit(this.canvas);
        }
        // Snap camera to head-on pose immediately
        if (typeof this.updateCameraPosition === 'function') {
            try { this.updateCameraPosition(); } catch (_) { /* camera may not exist yet */ }
        }
    }

    /**
     * Pointer orbit/zoom scoped to the visualizer canvas (embed).
     * Uses stopPropagation so the floating-panel drag does not steal the gesture.
     */
    bindEmbedCanvasOrbit(canvas) {
        if (!canvas || canvas.dataset.artef4ktOrbitBound === '1') return;
        canvas.dataset.artef4ktOrbitBound = '1';
        ...
        const onDown = (e) => {
            if (!this.cameraOrbitEnabled || !this.cameraControls) return;
            // Left button only; allow middle/right for future pan
        ...
        const onMove = (e) => {
            if (!dragging || !this.cameraControls) return;
            if (pointerId != null && e.pointerId !== pointerId) return;
            const dx = e.clientX - lastX;
            const dy = e.clientY - lastY;
            lastX = e.clientX;
            lastY = e.clientY;
            this.cameraControls.azimuth -= dx * 0.45;
            this.cameraControls.elevation += dy * 0.45;
        ...
        const onUp = (e) => {
            ...
            dragging = false;
            pointerId = null;
            canvas.style.cursor = this.cameraOrbitEnabled ? 'grab' : 'default';
            try { canvas.releasePointerCapture(e.pointerId); } catch (_) { /* ignore */ }
            e.stopPropagation();
        };
        const onWheel = (e) => {
            if (!this.cameraOrbitEnabled || !this.cameraControls) return;
            // Scale zoom step with current distance so zoom-out stays responsive far away
            const dist = Math.max(1, this.cameraControls.distance || 16);
            const zoomSpeed = 0.015 * (0.5 + dist / 20);
            this.cameraControls.distance += e.deltaY * zoomSpeed;
            // Refresh min; keep max at least getMaxDistance (and any higher user value)
            ...
            this.cameraControls.maxDistance = Math.max(
                ...
                this.cameraControls.distance || 0,
            );
        ...
        this._embedOrbitCleanup = () => {
            canvas.removeEventListener('pointerdown', onDown);
            canvas.removeEventListener('pointermove', onMove);
            canvas.removeEventListener('pointerup', onUp);
            canvas.removeEventListener('pointercancel', onUp);
            canvas.removeEventListener('wheel', onWheel);
            delete canvas.dataset.artef4ktOrbitBound;
        };
    }

    /**
     * Restore default static head-on view: no auto orbit / fly-around,
     * azimuth & elevation 0, base distance, target at origin.
     */
    resetCameraPose() {
        if (!this.cameraControls) return;
        const dist = this.getBaseDistance();
        this.cameraControls.minDistance = this.getMinDistance();
        this.cameraControls.maxDistance = this.getMaxDistance();
        this.cameraControls.distance = dist;
        this.cameraControls.azimuth = 0;
        this.cameraControls.elevation = 0; // head-on
        this.cameraControls.targetX = 0;
        this.cameraControls.targetY = 0;
        this.cameraControls.targetZ = 0;
        if (this.cameraControls.autoMovement) {
            const am = this.cameraControls.autoMovement;
            am.enabled = false; // static default
            am.manualOverride = false;
            am.overrideTimeout = 0;
            am.baseDistance = dist;
            am.baseAzimuth = 0;
            am.baseElevation = 0;
            am.rotationPhase = 0;
            if (am.flyAround) {
                am.flyAround.enabled = false;
                am.flyAround.isActive = false;
                am.flyAround.cooldownTime = 0;
                am.flyAround.lastIntensityPeak = 0;
            }
        }
        if (typeof this.updateCameraPosition === 'function') {
            try { this.updateCameraPosition(); } catch (_) { /* ignore */ }
        }
    }
    ...
    getMaxDistance() {
        // Allow a wide pull-back for embed / Controls zoom-out (was capped ~20–23)
        if (this.gridSize <= 15) {
            return 72 + (this.gridSize - 10) * 2.5; // ~72–84.5
        }
        return 90 + Math.min(40, (this.gridSize - 15) * 1.5); // grows with large grids
    }
    ...
    updateFlyAroundSystem(totalIntensity, currentTime, deltaTime) {
        ...
        if (!flyAround || flyAround.enabled === false) {
            if (flyAround) flyAround.isActive = false;
            return;
        }
    ...
    updateNormalCameraMovement(deltaTime, speedMultiplier, currentTime) {
        ...
        const am = this.cameraControls.autoMovement || {};
        // User knobs (Controls → Engine → Camera). Defaults match initEmbedCameraControls.
        const rotScale = Number.isFinite(Number(am.rotationSpeed)) ? Number(am.rotationSpeed) : 0.8;
        const elevScale = (Number.isFinite(Number(am.elevationIntensity)) ? Number(am.elevationIntensity) : 5) / 5;
        const zoomScale = (Number.isFinite(Number(am.zoomIntensity)) ? Number(am.zoomIntensity) : 2) / 2;
        
        // Enhanced horizontal rotation - continuous orbital movement with music bursts
        const baseRotationSpeed = 1.2 * speedMult * rotScale;
        const musicRotationBoost = totalIntensity * 8 * speedMult * rotScale;
        ...
        const directionVariation = Math.sin(timePhase * 0.3) * totalIntensity * 3 * rotScale;
        ...
        const musicElevationInfluence = totalIntensity * 20 * elevScale;
        
        // Create sweeping elevation movements that favor side views
        const baseElevationSweep = Math.sin(elevationTimePhase) * 25 * elevScale + 10;
        const musicElevationSweep = Math.sin(elevationTimePhase * 1.5) * musicElevationInfluence * 0.6;
        const bassElevationPulse = this.bassIntensity * Math.sin(elevationTimePhase * 3) * 15 * elevScale;
        ...
        const scaledBaseDistance = (am.baseDistance != null && Number.isFinite(Number(am.baseDistance))
            ? Number(am.baseDistance)
            : 16 * gridSizeScale);
        
        // Scale effects consistently - don't increase for larger grids
        const effectScale = gridSizeScale * zoomScale;
        ...
        const bassZoomPulse = this.bassIntensity * Math.sin(distanceTimePhase * 4) * 2 * effectScale;
        ...
        // Respect live max (may be raised by Controls Distance scrub). Do not
        // pull a user-set distance back down every auto-move frame.
        const minDistance = this.cameraControls.minDistance != null
            ? this.cameraControls.minDistance
            : this.getMinDistance();
        let maxDistance = this.cameraControls.maxDistance != null
            ? this.cameraControls.maxDistance
            : this.getMaxDistance();
        maxDistance = Math.max(maxDistance, this.getMaxDistance());
    ...
    updateCameraPosition() {
        ...
        // Standalone only: keep camera inside grid "room".
        // Embed (music_view): free zoom — wall clamp was capping distance at ~gridSize (~15)
        // even when Controls Distance was set much higher.
        if (!this.embedMode) {
            const margin = 1.5;
        ...
        // Snap on Controls distance/pose changes; otherwise light damping
        if (this._cameraSnap || this.embedMode) {
            // Embed: follow distance 1:1 (damping made zoom feel broken / laggy)
    ...
    calculateMaxSafeDistance(center, direction, controls) {
      ...
      animate() {
        ...
        // Update time display and track progress (standalone UI only)
        if (!this.embedMode) {
            this.updateTimeDisplay();
        }
    ...
    resize() {
        const width = this.embedMode ? this.embedWidth : window.innerWidth;
        const height = this.embedMode ? this.embedHeight : window.innerHeight;
        ...
        if (this.renderer) {
            // updateStyle=false in embed so CSS 100% fill stays controlled by host
            this.renderer.setSize(width, height, !this.embedMode);
            this.renderer.setPixelRatio(Math.min(window.devicePixelRatio || 1, this.embedMode ? 1.5 : 2));
        }
        
        // Update filmic tone system for new size
        if (this.filmicToneSystem && typeof this.filmicToneSystem.onWindowResize === 'function') {
    ...
    getUISettings() {
        return {
            ...
            // Light colors + enables
            lightBassColor: this.lightBassColor,
            lightMidColor: this.lightMidColor,
            lightHighColor: this.lightHighColor,
            lightsEnabled: this.lightsEnabled !== false,
            lightKeyEnabled: this.lightKeyEnabled !== false,
            lightBassEnabled: this.lightBassEnabled !== false,
            lightMidEnabled: this.lightMidEnabled !== false,
            lightHighEnabled: this.lightHighEnabled !== false,
            ...
            // Camera (auto motion + pose + embed orbit)
            cameraOrbitEnabled: this.cameraOrbitEnabled !== false,
            cameraAutoMove: !!(this.cameraControls?.autoMovement?.enabled),
            cameraRotationSpeed: this.cameraControls?.autoMovement?.rotationSpeed ?? 0.8,
            cameraZoomIntensity: this.cameraControls?.autoMovement?.zoomIntensity ?? 2,
            cameraElevationIntensity: this.cameraControls?.autoMovement?.elevationIntensity ?? 5,
            cameraFlyAround: !!(this.cameraControls?.autoMovement?.flyAround?.enabled),
            cameraFlyPeak: this.cameraControls?.autoMovement?.flyAround?.peakThreshold ?? 0.75,
            cameraFlySpeed: this.cameraControls?.autoMovement?.flyAround?.speedMultiplier ?? 2.5,
            cameraOverrideSeconds: (this.cameraControls?.autoMovement?.overrideDuration ?? 3000) / 1000,
            cameraAzimuth: this.cameraControls?.azimuth ?? 0,
            cameraElevation: this.cameraControls?.elevation ?? 0,
            cameraDistance: this.cameraControls?.distance ?? this.getBaseDistance(),
    ...
    applyUISettings(settings) {
        try {
            ...
            // Light enable flags
            let lightFlagsChanged = false;
            if (settings.lightsEnabled !== undefined) {
                this.lightsEnabled = !!settings.lightsEnabled;
                lightFlagsChanged = true;
            }
            if (settings.lightKeyEnabled !== undefined) {
                this.lightKeyEnabled = !!settings.lightKeyEnabled;
                lightFlagsChanged = true;
            }
            if (settings.lightBassEnabled !== undefined) {
                this.lightBassEnabled = !!settings.lightBassEnabled;
                lightFlagsChanged = true;
            }
            if (settings.lightMidEnabled !== undefined) {
                this.lightMidEnabled = !!settings.lightMidEnabled;
                lightFlagsChanged = true;
            }
            if (settings.lightHighEnabled !== undefined) {
                this.lightHighEnabled = !!settings.lightHighEnabled;
                lightFlagsChanged = true;
            }
            if (lightFlagsChanged && typeof this.applyLightEnableState === 'function') {
                this.applyLightEnableState();
            }
            ...
            if (settings.envSphereSize !== undefined) {
              ...
              // UI opacity — only when provided (partial host patches must not reset to 1.0)
            ...
            // Camera controls (music_view Engine panel + embed orbit)
            if (this.cameraControls) {
                const am = this.cameraControls.autoMovement || (this.cameraControls.autoMovement = {});
                if (!am.flyAround) {
                    am.flyAround = {
                        isActive: false, enabled: true, startTime: 0, duration: 0,
                        lastIntensityPeak: 0, peakThreshold: 0.75, minDuration: 2000,
                        maxDuration: 4000, cooldownTime: 0, cooldownDuration: 3000, speedMultiplier: 2.5,
                    };
                }
                if (settings.cameraOrbitEnabled !== undefined) {
                    this.cameraOrbitEnabled = !!settings.cameraOrbitEnabled;
                    if (this.canvas) {
                        this.canvas.style.cursor = this.cameraOrbitEnabled ? 'grab' : 'default';
                    }
                }
                if (settings.cameraAutoMove !== undefined) {
                    am.enabled = !!settings.cameraAutoMove;
                }
                if (settings.cameraRotationSpeed !== undefined) {
                    am.rotationSpeed = Number(settings.cameraRotationSpeed);
                }
                if (settings.cameraZoomIntensity !== undefined) {
                    am.zoomIntensity = Number(settings.cameraZoomIntensity);
                }
                if (settings.cameraElevationIntensity !== undefined) {
                    am.elevationIntensity = Number(settings.cameraElevationIntensity);
                }
                if (settings.cameraFlyAround !== undefined) {
                    am.flyAround.enabled = !!settings.cameraFlyAround;
                    if (!am.flyAround.enabled) am.flyAround.isActive = false;
                }
                if (settings.cameraFlyPeak !== undefined) {
                    am.flyAround.peakThreshold = Number(settings.cameraFlyPeak);
                }
                if (settings.cameraFlySpeed !== undefined) {
                    am.flyAround.speedMultiplier = Number(settings.cameraFlySpeed);
                }
                if (settings.cameraOverrideSeconds !== undefined) {
                    am.overrideDuration = Math.max(0.2, Number(settings.cameraOverrideSeconds)) * 1000;
                }
                if (settings.cameraAzimuth !== undefined) {
                    this.cameraControls.azimuth = Number(settings.cameraAzimuth);
                    am.manualOverride = true;
                    am.overrideTimeout = performance.now();
                }
                if (settings.cameraElevation !== undefined) {
                    this.cameraControls.elevation = Math.max(
                        this.cameraControls.minElevation,
                        Math.min(this.cameraControls.maxElevation, Number(settings.cameraElevation)),
                    );
                    am.manualOverride = true;
                    am.overrideTimeout = performance.now();
                }
                if (settings.cameraDistance !== undefined) {
                    // Keep limits in sync; allow Controls values up to slider max (~120)
                    this.cameraControls.minDistance = this.getMinDistance();
                    const want = Number(settings.cameraDistance);
                    const floorMax = this.getMaxDistance();
                    this.cameraControls.maxDistance = Math.max(
                        floorMax,
                        Number.isFinite(want) ? want : floorMax,
                        this.cameraControls.maxDistance || 0,
                    );
                    this.cameraControls.distance = Math.max(
                        this.cameraControls.minDistance,
                        Math.min(this.cameraControls.maxDistance, want),
                    );
                    am.manualOverride = true;
                    am.overrideTimeout = performance.now();
                    // Prevent auto-move from overwriting distance next frames while scrubbing
                    if (am.enabled) {
                        am.baseDistance = this.cameraControls.distance;
                    }
                    this._cameraSnap = true;
                    if (typeof this.updateCameraPosition === 'function') {
                        try { this.updateCameraPosition(); } catch (_) { /* ignore */ }
                ...
                if (settings.cameraAzimuth !== undefined || settings.cameraElevation !== undefined) {
                    this._cameraSnap = true;
                    if (typeof this.updateCameraPosition === 'function') {
                        try { this.updateCameraPosition(); } catch (_) { /* ignore */ }
                    }
                }
                if (settings.cameraReset) {
                    this.resetCameraPose();
                    this._cameraSnap = true;
                    // Pose reset is authoritative for auto/fly flags; re-apply explicit
                    // camera* keys from this same patch after if they were also sent.
                    if (settings.cameraAutoMove !== undefined) {
                        this.cameraControls.autoMovement.enabled = !!settings.cameraAutoMove;
                    }
                    if (settings.cameraFlyAround !== undefined && this.cameraControls.autoMovement.flyAround) {
                        this.cameraControls.autoMovement.flyAround.enabled = !!settings.cameraFlyAround;
                        if (!settings.cameraFlyAround) {
                            this.cameraControls.autoMovement.flyAround.isActive = false;
                        }
                    }
                    if (typeof this.updateCameraPosition === 'function') {
                        try { this.updateCameraPosition(); } catch (_) { /* ignore */ }
            ...
            // Mouse interaction is always enabled during idle mode
              // Grid Cells Activity — only when provided
            ...
            if (debugPanel && debugConsoleToggle) {
              ...
              // Shockwave — only apply keys that are present (partial patches must not reset)
            if (settings.shockwaveEnabled !== undefined) {
                const shockwaveEnabled = !!settings.shockwaveEnabled;
                const shockwaveEnabledToggle = document.getElementById('shockwave-enabled');
                if (shockwaveEnabledToggle) shockwaveEnabledToggle.checked = shockwaveEnabled;
                if (this.shockwaveSystem) {
                    this.shockwaveSystem.setEnabled(shockwaveEnabled);
                }
            }
            
            if (settings.shockwaveIntensity !== undefined && this.shockwaveSystem) {
                const shockwaveIntensity = settings.shockwaveIntensity;
                const shockwaveIntensitySlider = document.getElementById('shockwave-intensity');
                const shockwaveIntensityValue = document.getElementById('shockwave-intensity-value');
                if (shockwaveIntensitySlider) shockwaveIntensitySlider.value = shockwaveIntensity;
                if (shockwaveIntensityValue) shockwaveIntensityValue.textContent = Number(shockwaveIntensity).toFixed(1);
                this.shockwaveSystem.config.intensity = shockwaveIntensity;
            }
            
            if (settings.shockwaveLineCount !== undefined && this.shockwaveSystem) {
                const shockwaveLineCount = settings.shockwaveLineCount;
            ...
            if (settings.shockwaveExpansionSpeed !== undefined && this.shockwaveSystem) {
                const shockwaveExpansionSpeed = settings.shockwaveExpansionSpeed;
                ...
                if (shockwaveSpeedValue) shockwaveSpeedValue.textContent = Number(shockwaveExpansionSpeed).toFixed(1);
                this.shockwaveSystem.config.expansionSpeed = shockwaveExpansionSpeed;
            }
            
            if (settings.shockwaveLifetime !== undefined && this.shockwaveSystem) {
                const shockwaveLifetime = settings.shockwaveLifetime;
                const shockwaveLifetimeSlider = document.getElementById('shockwave-lifetime');
                const shockwaveLifetimeValue = document.getElementById('shockwave-lifetime-value');
                if (shockwaveLifetimeSlider) shockwaveLifetimeSlider.value = shockwaveLifetime;
                if (shockwaveLifetimeValue) shockwaveLifetimeValue.textContent = Number(shockwaveLifetime).toFixed(1);
                this.shockwaveSystem.config.lifetime = shockwaveLifetime;
            }
            
            if (settings.shockwaveOpacity !== undefined && this.shockwaveSystem) {
                const shockwaveOpacity = settings.shockwaveOpacity;
                const shockwaveOpacitySlider = document.getElementById('shockwave-opacity');
                const shockwaveOpacityValue = document.getElementById('shockwave-opacity-value');
                if (shockwaveOpacitySlider) shockwaveOpacitySlider.value = shockwaveOpacity;
                if (shockwaveOpacityValue) shockwaveOpacityValue.textContent = Number(shockwaveOpacity).toFixed(1);
            ...
            if (this.filmicToneSystem) {
                ...
                } else if (settings._partial !== true) {
                    // Full preset load without filmic block → defaults.
                    // Partial host patches set _partial so we leave filmic alone.
...
(() => {
    ...
    const uiPanel = document.getElementById('ui');
    const uiHoverArea = document.getElementById('ui-hover-area');
    // Embed host has no ARTEF4KT chrome
    if (!uiPanel) return;
...
// Initialize the visualizer when the page loads (standalone only).
// music_view sets window.ARTEF4KT_NO_AUTO_INIT = true and constructs embeds via createArtef4ktEmbed.
if (!window.ARTEF4KT_NO_AUTO_INIT) {
...
// Expose class for embed hosts
window.FerrofluidVisualizer = FerrofluidVisualizer;
